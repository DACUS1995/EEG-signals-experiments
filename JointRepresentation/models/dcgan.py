...
def define_discriminator(in_shape=(56,56,3), in_shape_eeg_features=512):
	# label input
	...
	out_layer = tf.keras.layers.Dropout(0.4)(out_layer)
...
def define_generator(latent_dim=100, in_shape_eeg_features=512):
	...
	gen = tf.keras.layers.Reshape((7, 7, 256))(gen)
	...
	# output
	out_layer = tf.keras.layers.Conv2D(3, (7,7), activation='sigmoid', padding='same')(gen)
	# define model
